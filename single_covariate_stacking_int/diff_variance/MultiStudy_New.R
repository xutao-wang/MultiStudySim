...
MultiStudySim_2 <- function(k,nk,m,mu_x,sigma_x,pii,beta_vec){
  ...
  x_list <- lapply(1:length(x_vec_list), function(x) data.frame(X=x_vec_list[[x]],X1=x_vec_ind[[x]]))
...
dat_input_0.1 <- lapply(1:50,function(x) MultiStudySim_2(k,nk,m,mu_x,sigma_x_0.1,pii,beta_vec))
dat_input_0.5 <- lapply(1:50,function(x) MultiStudySim_2(k,nk,m,mu_x,sigma_x_0.5,pii,beta_vec))
dat_input_1 <- lapply(1:50,function(x) MultiStudySim_2(k,nk,m,mu_x,sigma_x_1,pii,beta_vec))
dat_input_4 <- lapply(1:50,function(x) MultiStudySim_2(k,nk,m,mu_x,sigma_x_4,pii,beta_vec))
...
boost_class_all <- function(lst,dat_train,dat_test,nk_train){
  con <- caret::trainControl(method = 'cv',number=5,allowParallel=T)
  fit <- lapply(lst,function(lst) caret::train(y~X+X:X1, data=lst, method = "gbm",trControl=con,preProc = c("center", "scale")))
  #insamp_rmse_list <- lapply(1:length(fit),function(x) 
  #  Metrics::rmse(predict(fit[[x]],lst[[x]][,-which(names(lst[[x]]) %in% c('y'))]),lst[[x]]$y))
...
#saveRDS(case_0.1, file = "~/Desktop/master_project/base_case/10s_10p_new_way/diff_variance/case_0.1.rds")
saveRDS(case_0.1, file = "~/MultiSim/result/10s_10p_new_way/diff_variance/case_0.1.rds")
...
#saveRDS(case_0.5, file = "~/Desktop/master_project/base_case/10s_10p_new_way/diff_variance/case_0.5.rds")
saveRDS(case_0.5, file = "~/MultiSim/result/10s_10p_new_way/diff_variance/case_0.5.rds")
...
#saveRDS(case_1, file = "~/Desktop/master_project/base_case/10s_10p_new_way/diff_variance//case_1.rds")
saveRDS(case_1, file = "~/MultiSim/result/10s_10p_new_way/diff_variance/case_1.rds")
...
#saveRDS(case_4, file = "~/Desktop/master_project/base_case/10s_10p_new_way/diff_variance/case_4.rds")
saveRDS(case_4, file = "~/MultiSim/result/10s_10p_new_way/diff_variance/case_4.rds")
