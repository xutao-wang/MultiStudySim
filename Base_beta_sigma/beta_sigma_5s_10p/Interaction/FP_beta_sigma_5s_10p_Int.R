case_0 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_0.RDS')
case_0.25 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_0.25.RDS')
case_0.5 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_0.5.RDS')
case_0.75 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_0.75.RDS')

case_1 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_1.RDS')
case_1.5 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_1.5.RDS')
case_2 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_2.RDS')
case_2.5 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_2.5.RDS')
case_3 <- readRDS('~/Desktop/master_project/base_case/Base_beta_sigma/beta_sigma_5s_10p/Interaction/FiveInt/case_3.RDS')
...
get_long_data <- function(output,size){
  tt_elnet <- cbind(tidyr::gather(data.frame(do.call(rbind,lapply(output,'[[',1)))[,c(1:8)], weighting, measurement, stacking:merge, factor_key=TRUE),'elnet')
  tt_nnet <- cbind(tidyr::gather(data.frame(do.call(rbind,lapply(output,'[[',2)))[,c(1:8)], weighting, measurement, stacking:merge, factor_key=TRUE),'Nnet')
  ...
  tt_boost <- cbind(tidyr::gather(data.frame(do.call(rbind,lapply(output,'[[',3)))[,c(1:8)], weighting, measurement, stacking:merge, factor_key=TRUE),'boost')
  
  tt_rf <- cbind(tidyr::gather(data.frame(do.call(rbind,lapply(output,'[[',4)))[,c(1:8)], weighting, measurement, stacking:merge, factor_key=TRUE),'rf')
...
get_learner_plotting(dat_total,'Beta Value [-3,-2]&[2,3],5S_10P_FiveInteraction','Beta Sigma')
...
get_plot_ratio <- function(dat_total,learner){
  ...
  dat_plot_ratio <- dat_plot[-which(dat_plot$weighting %in% 'stacking'),]
  base <- dat_plot$measurement[which(dat_plot$weighting %in% 'stacking')]
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'stacking_wz'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'stacking_wz'),]$measurement-base
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'stacking_int'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'stacking_int'),]$measurement-base
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'elnet_int'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'elnet_int'),]$measurement-base
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'lasso_noint'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'lasso_noint'),]$measurement-base
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'ridge_noint'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'ridge_noint'),]$measurement-base
  
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'simp_avg'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'simp_avg'),]$measurement-base
  dat_plot_ratio[which(dat_plot_ratio$weighting %in% 'merge'),]$measurement <- dat_plot[which(dat_plot$weighting %in% 'merge'),]$measurement-base
...
grid.arrange(get_plot_ratio(dat_total,'elnet'),get_plot_ratio(dat_total,'Nnet'),
             ...
             ncol=2,top=textGrob('Beta Value [-3,-2] & [2,3] 5S_10P_FiveInteraction',gp=gpar(fontsize=20,font=3)))
